...
function renderUsers(container) {
  ...
  var html =
    ...
    '<div id="admin-user-table-wrap">' + buildUserTable(users, '') + '</div>';
  container.innerHTML = html;
  document.getElementById('admin-user-search')?.addEventListener('input', function () {
    document.getElementById('admin-user-table-wrap').innerHTML = buildUserTable(getUsers(), this.value);
  });
}

function buildUserTable(users, query) {
  var filtered = users;
  if (query) {
  ...
  var rows = filtered.map(function (u) {
    ...
    return '<tr>' +
      '<td><span class="admin-user-icon">' + escapeHtml(u.icon || u.username.charAt(0).toUpperCase()) + '</span> ' + escapeHtml(u.username) + '</td>' +
      '<td>' + escapeHtml(u.email || '-') + '</td>' +
      '<td>' + formatDate(u.createdAt) + '</td>' +
      '<td class="admin-actions-cell">' +
        '<button class="btn btn-sm btn-ghost admin-action-btn" data-action="view" data-user="' + escapeHtml(u.username) + '">Ver</button>' +
        (isSelf ? '' : '<button class="btn btn-sm btn-delete admin-action-btn" data-action="delete" data-user="' + escapeHtml(u.username) + '">Eliminar</button>') +
      '</td></tr>';
  }).join('');
